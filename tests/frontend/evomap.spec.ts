...
const MIME: Record<string, string> = { ".html": "text/html; charset=utf-8", ".css": "text/css", ".js": "text/javascript", ".mjs": "text/javascript", ".gif": "image/gif", ".svg": "image/svg+xml", ".ttf": "font/ttf", ".ico": "image/x-icon" };
...
test.beforeAll(async () => {
  server = createServer(async (req, res) => {
    ...
    if (url.startsWith("/zh/")) url = url; // pages live under community/zh
    else if (!url.startsWith("/community/")) { res.writeHead(404); res.end(); return; }
...
function stubApi(page: Page, opts: { boardStatus?: number } = {}) {
  return Promise.all([
    ...
    page.route("**/api/gongzhi/threads/*", r => r.fulfill({ json: { ok: true, mode: "live", data: { thread_id: "n1", records: [need, reply], next_cursor: null, mode: "live" } } })),
...
test.describe("EvoMap 静态前端共治适配", () => {
  test("主页：原层次 + 共治叙事 + 真实公告与线程，无第三方请求", async ({ page }) => {
    await stubApi(page);
    ...
    await page.locator("#board .cm-record", { hasText: "先确认活动边界" }).click();
    await expect(page.locator(".cm-dialog")).toContainText("第一次办 AI 体验活动");
    await expect(page.locator(".cm-dialog .cm-thread-record")).toHaveCount(2);
    await page.getByRole("button", { name: "关闭面板" }).click();
    // Agent 点图：chips 与说明，不代表在线
