...
/** 由 I 的隔离 HTTP/PG runner 注册；login 必须走真实 start/callback/session 路由。
 * 上游若为 fixture 必须明确标注，不得在生产 runtime 引入测试身份后门。
 * 原四项邮箱流程的后续回归；只替换登录驱动，保留实际授权/内容批准/版本下载。
 * 本文件不读取账号凭据，不自行设置 Cookie，不启动服务或连接数据库。 */
export function defineOAuthBrowserFlows(options: {
  base: string;
  evidence: string;
  login: (page: Page, account: "A" | "B") => Promise<void>;
}) {
  const url = new URL(options.base);
  if (url.protocol !== "http:" || url.hostname !== "127.0.0.1" || !url.port || url.pathname !== "/") throw new Error("OAuth protocol suite requires an explicitly assigned loopback server");
  const BASE = url.origin, evidence = options.evidence;
  if (process.env.GONGZHI_FRONTEND_SOURCE_OVERLAY === "true") throw new Error("OAuth protocol acceptance requires actually hosted assets");
  mkdirSync(evidence, { recursive: true });
  test.beforeEach(async ({ request }) => {
    const response = await request.get(BASE + "/api/gongzhi/config");
    expect(response.status()).toBe(200);
    const cfg = await response.json();
    expect(cfg.mode).toBe("live"); expect(cfg.data.auth.provider).toBe("zhihu");
    expect(cfg.data.auth.available).toBe(true); expect(cfg.data.auth.url).toBeNull();
    expect(cfg.data.auth.public_key).toBeNull(); expect(cfg.data.database_configured).toBe(true);
  });
  // 一次性授权显示时不截图，失败关闭页防止自动 error-context 记录秘密。
...
async function login(page: Page, account: "A" | "B", name: string) {
  await options.login(page, account);
...
test("托管协议：登录/绑定/签发撤销授权/发布/回复/平台如实失败/退出", async ({ page }) => {
  await login(page, "A", NAME_A);
  ...
  await expect(page.locator("[data-cm-account]").getByRole("button", { name: "使用知乎登录", exact: true })).toBeVisible();
...
test("托管协议：无效回调不得创建登录状态", async ({ page }) => {
  await page.goto(BASE + "/auth/zhihu/callback?state=invalid-browser-fixture&code=invalid-browser-fixture");
  ...
  await expect(page.getByRole("button", { name: "使用知乎登录", exact: true })).toBeVisible();
});
...
test("真实托管双账号：准确批准分享、作者撤销后借用、独立反馈批准与回读", async ({ page, request }) => {
  ...
  // 站内 Cookie 会话跨标签退出：旧标签的未上传本地草稿不得留给下一身份。
  await page.goto(BASE + "/zh/board/");
  await expect(page.locator(".cm-publish-bar")).toBeVisible();
  await page.getByRole("button", {name:"整理本地草稿 / 导入 SKILL.md"}).click();
