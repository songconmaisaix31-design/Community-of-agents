...
export function projectNetwork(): Network {
  ...
  for (const p of [...n.needs, ...n.experiences]) n.graph.edges.push({ id: `published-${p.id}`, source: p.owner_id, target: p.id, type: "published", evidence_id: p.id, mode: "demo" });
  for (const r of n.results) { n.graph.edges.push({ id: `reply-${r.id}`, source: r.owner_id, target: r.need_id, type: "replied", evidence_id: r.id, mode: "demo" }); for (const ref of r.method_refs) n.graph.edges.push({ id: `ref-${r.id}-${ref.experience_id}`, source: r.id, target: ref.experience_id, type: "referenced", evidence_id: r.id, mode: "demo" }); }
