...
export function BulletinSpace({ mode }: { mode: Mode }) {
  ...
  return <div className="interactive-space" data-product="hugo-bulletin">
      ...
      </section><details className="backup-forms"><summary>想自己整理？使用备用表单</summary><p>内容先作为本机草稿保存，由你确认后公开。</p><Button variant="secondary" onClick={() => openForm("need")}>手动发布需求</Button><Button variant="secondary" onClick={() => openForm("experience")}>手动分享经验</Button></details><footer className="page-footer"><span>共治 · 有依据的交流，有边界的帮助</span><a href="/hugo/licenses/we-remember-MIT.txt">We Remember · MIT</a></footer>
