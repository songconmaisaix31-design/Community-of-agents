...
export function AgentCanvas({ graph, selected, highlighted, onSelect, onEvidence }: { graph: AgentGraph; selected: string | null; highlighted: Set<string> | null; onSelect: (id: string) => void; onEvidence: (edge: AgentGraphEdge) => void }) {
  ...
  useEffect(() => {
    let dead = false, local: Cosmos | undefined;
    const timeout = setTimeout(() => { if (!dead) { setFailed(true); dead = true; if (local?.isReady) local.destroy(); instance.current = null; } }, 12000);
    void (async () => { try {
      const { Graph } = await import("@cosmos.gl/graph"); if (dead || !host.current) return;
      local = new Graph(host.current, { backgroundColor: "#f8f5ef", pointDefaultSize: 4, pointDefaultColor: "#587267", linkDefaultColor: "#91a49b", linkDefaultWidth: 0.65, hoveredLinkColor: "#c94d3d", hoveredLinkWidthIncrease: 3, simulationRepulsion: 1.6, simulationLinkDistance: 110, simulationDecay: 150, simulationRepulsionFromMouse: 0, enableDrag: false, fitViewOnInit: false, randomSeed: 27,
      ...
      await local.ready; if (dead) { local.destroy(); return; } instance.current = local; clearTimeout(timeout); setReady(true);
    } catch { if (!dead) setFailed(true); clearTimeout(timeout); if (local?.isReady) local.destroy(); } })();
    return () => { dead = true; clearTimeout(timeout); instance.current = null; if (local?.isReady) local.destroy(); };
  }, []);
  useEffect(() => {
    ...
    const positions = new Float32Array(ids.current.flatMap((id, i) => !byId.has(id) ? [NaN, NaN] : previousIds.has(id) && Number.isFinite(existing[i * 2]) ? [existing[i * 2], existing[i * 2 + 1]] : [Math.sin(i * 13.7 + 1) * 180, Math.cos(i * 7.3 + 1) * 180]));
