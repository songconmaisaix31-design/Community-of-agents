...
随后在本机使用无回显标准输入比对用户先前提供的 Key 与项目配置：**两者完全一致**，没有输出 Key、没有额外网络请求、没有替换配置。因而不再请求用户重发相同 Key；需要在开放平台确认5,000额度是否已绑定到该 Key 的搜索/回答能力，或提供实际具有对应额度的新配置。不能猜测账户状态或以用户口头额度替代接口计数，3次请求统计保持不变。原生OpenCode读取确认仍需用户完成，M已切换到对应终端面板。
