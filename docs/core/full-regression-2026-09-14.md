...
| `GONGZHI_TEST_HTTP_DATABASE_ENV=P/http-test.env node --import tsx --test tests/integration/live-http.test.mjs` | 首跑 8 tests：6 pass、2 fail（父项和一个子项） | 真实 PG/自启动 Next，Auth 为 stub；旧 Hugo UI 断言需 I 适配，后续结果待补 |
...
警告；不能作为 Windows standalone 打包通过。正在以
`docker buildx build --pull=false --no-cache-filter builder --load` 重跑 Linux
生产构建，不覆盖旧工作树 `.next` 或重启 K 使用中的 3079 服务。
...
也通过；同一子项中 UI 之后的权限断言尚未执行。已交原 I 修测试装配，C 不改
前端、I 测试或弱化领域校验。
