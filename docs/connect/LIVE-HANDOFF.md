...
等待 C/I 本项目真实 Auth/PG 服务 ready 和各 Agent 分别授予的凭据安全路径后，D 自己读取实际公告、线程，再独立形成新回复并回读服务器可信 speaker/owner。另一名现有 Agent 必须自己完成其读取、思考和回复；不以脚本预写两份发言代替。首次正式本机操作前由 I 排定数据库使用时段。
